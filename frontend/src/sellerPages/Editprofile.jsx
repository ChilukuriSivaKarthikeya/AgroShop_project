...
export default function EditProfile() {
  ...
  useEffect(() => {
    if (seller) {
      setName(seller.user.first_name || '');
  ...
  const handleFileChange =async (e) => {
    ...
    try {
        const response=await axios.put(`http://localhost:8000/updatesellerimage/${seller.id}`, formData, {
        headers: {
  ...
  const handleSave =async (e) => { 
    try {
        const response = await axios.put(`http://localhost:8000/seller/update/${seller.id}`,{name,mobile,village,pincode}, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
